...
// Store large arrays in flash memory (read-only)
const char* ZhToPY::pinyinArrStr = 
...
const char* ZhToPY::firstLetterStr = 
...
std::vector<std::string> split_name(const std::string &name) {
    ...
    for (size_t j = 0; j < name.length();) {
        size_t cplen = 1;
        if ((name[j] & 0xf8) == 0xf0) cplen = 4;
        else if ((name[j] & 0xf0) == 0xe0) cplen = 3;
        else if ((name[j] & 0xe0) == 0xc0) cplen = 2;
        if ((j + cplen) > name.length()) cplen = 1;
...
static bool utf8_to_unicode(const std::string &utf8, std::vector<unsigned int> &unicode) {
    ...
    while (utf8_cursor < utf8length) {
        unsigned char* p = (unsigned char*)&utf8[utf8_cursor];
        unsigned int e = 0;
...
int get_code_point(const std::string &name) {
    ...
    return unicode.empty() ? 0 : unicode[0];
}

// Constructor
ZhToPY::ZhToPY() {
    // Split pinyin array (stored in flash) into vector
...
std::string ZhToPY::zhToPY(const std::string &chinese) {
    ...
    for (const auto &w : words) {
        ...
        if (unicode >= 0x4E00 && unicode <= 0x9FA5) {
            size_t index = unicode - 0x4E00;
            if (index < listPY.size()) {
                result.append(listPY[index]);
            }
        } else {
            result.append(w);
        }
...
std::string ZhToPY::zhToJP(const std::string &chinese) {
    ...
    std::string result;
    std::vector<std::string> words = split_name(chinese);
     // Cache length once (static ensures it's only calculated on first call)
    static const size_t firstLetterStrLen = strlen(firstLetterStr);
    
    for (const auto &w : words) {
        int unicode = get_code_point(w);
        
        // Pass through ASCII characters (letters and numbers)
        if ((unicode >= 'a' && unicode <= 'z') || 
            (unicode >= 'A' && unicode <= 'Z') ||
            (unicode >= '0' && unicode <= '9')) {
            result.append(w);
        } else if (unicode >= 0x4E00 && unicode <= 0x9FA5) {
            int index = unicode - 19968;
            // Use cached length instead of strlen
            if (index >= 0 && index < static_cast<int>(firstLetterStrLen)) {
                result.push_back(firstLetterStr[index]);
            }
        }
    }
    return result;
...
std::string ZhToPY::zhToZM(const std::string &chinese) {
    std::string jp = zhToJP(chinese);
    return jp.empty() ? "" : std::string(1, jp[0]);
}